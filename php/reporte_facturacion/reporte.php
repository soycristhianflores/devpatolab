...
if($type == 1 || $type == 2 || $type == 4){//SUPER ADMINISTRADOR, ADMINISTRADOR Y CONTADOR GENERAL
	...
	if($profesional == ""){
		$where = "WHERE f.fecha BETWEEN '$desde' AND '$hasta' AND f.usuario = '$usuario' AND f.estado ".$in;
	}else{
		$where = "WHERE f.colaborador_id = '$profesional' AND f.fecha BETWEEN '$desde' AND '$hasta' AND f.usuario = '$usuario' AND f.estado ".$in;
	}
}
...
$registro = "SELECT f.fecha AS 'fecha', f.number As 'factura', SUM(f.importe) AS 'importe', p.identidad AS 'identidad', CONCAT(p.nombre,' ',p.apellido) AS 'paciente', CONCAT(c.nombre,'',c.apellido) AS 'profesional', DATE_FORMAT(f.fecha, '%d/%m/%Y') AS 'fecha1', f.pacientes_id AS 'pacientes_id', f.cierre AS 'cierre', f.servicio_id AS 'servicio_id', f.colaborador_id AS 'colaborador_id', f.fecha AS 'fecha_consulta', s.nombre AS 'servicio', (CASE WHEN f.tipo_factura = 1 THEN 'Contado' ELSE 'Crédito' END) AS 'tipo_documento'
	...
	".$where."
	GROUP BY f.number
	ORDER BY f.number, f.fecha DESC";
...
if($result->num_rows>0){
	while($registro2 = $result->fetch_assoc()){
		...
		if($profesional == ""){
			$where = "WHERE number = '$numero' AND fecha BETWEEN '$desde' AND '$hasta' AND estado NOT IN(1,3)";	
		}else{
			$where = "WHERE number = '$numero' AND colaborador_id = '$profesional' AND fecha BETWEEN '$desde' AND '$hasta' AND estado NOT IN(1,3)";
		}		
