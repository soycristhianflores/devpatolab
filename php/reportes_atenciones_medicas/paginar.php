...
$hasta = $_POST['hasta'];
$dato = $_POST['dato'];	
$profesional = $_POST['colaborador'];
$colaborador_id = $_SESSION['colaborador_id'];

if ($_SESSION['type']==6){
	if($profesional != ""){
		$where = "WHERE am.fecha BETWEEN '$desde' AND '$hasta' am.colaborador_id = '$colaborador_id'";
	}else if($dato != ""){
		...
		$where = "WHERE am.fecha BETWEEN '$desde' AND '$hasta' AND am.colaborador_id = '$colaborador_id'";
	}		
}else{
	if($profesional != ""){
...
while($registro2 = $result->fetch_assoc()){	
	$tabla = $tabla.'<tr>
	   ...
	   <td>
		  <a style="text-decoration:none;" data-toggle="tooltip" data-placement="right" href="javascript:mailAtencion('.$registro2['atencion_id'].');void(0);" class="far fa-paper-plane fa-lg" title="Enviar Reporte de Laboratorio"></a>	   
		  <a style="text-decoration:none;" data-toggle="tooltip" data-placement="right" title = "Imprimir Reporte" href="javascript:printReport('.$registro2['atencion_id'].','.$registro2['muestras_id'].');void(0);" class="fas fa-print fa-lg"></a>			  			  
		  <a style="text-decoration:none;" data-toggle="tooltip" data-placement="right" title = "Agregar Adendum" href="javascript:addAdendum('.$registro2['atencion_id'].','.$registro2['muestras_id'].');void(0);" class="fas fa-comment-medical fa-lg"></a> 
	   </td>	   
	</tr>';	        
}
