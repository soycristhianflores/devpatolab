...
if($servicio == "" && $colaborador == ""){
	$where = "WHERE am.fecha BETWEEN '$desde' AND '$hasta'";	
}else if($servicio != "" && $colaborador == "") {
	$where = "WHERE am.servicio_id = '$servicio' AND am.fecha BETWEEN '$desde' AND '$hasta'";	
}else{
	//OBTENER NOMBRE COLABORADOR/USUARIO DEL SISTEMA
	...
	$where = "WHERE am.servicio_id = '$servicio' AND am.colaborador_id = '$colaborador' AND am.fecha BETWEEN '$desde' AND '$hasta'";	
}
...
$fila=3;
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Atenciones ".$servicio_name);
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:AG$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:AG$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="REPORTE DIARIO DE MUESTRAS '.strtoupper($servicio_name).' '.strtoupper($mes).'_'.$año.'.xls"');
//**********************************************************************
